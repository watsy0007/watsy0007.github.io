...
处于安全原因, 禁止查看用户数据, 因此只能和用户多次沟通确认.

**2. 系统架构复杂 **

当前架构拆分多个子系统, 一个任务流转多个子系统, 全部正确执行才能得到最终结果.

具体解决方案这里就不展开, 涉及到我维护的系统部分拆分多个阶段实施, 第一阶段提供 “任务执行监控报告”.
具体点是提供任务每日执行情况可视化, 增强系统的可视化监控能力. 经过调研决定使用 Gantt 图展示所有任务的每日执行.
...
```{python}
#| echo: false
!pip install duckdb==1.0.0 plotly --quiet
```
...
本文主要介绍了如何使用 DuckDB 查找缺失的日期, 通过窗口函数和分组函数, 可以快速定位出缺失的日期.
