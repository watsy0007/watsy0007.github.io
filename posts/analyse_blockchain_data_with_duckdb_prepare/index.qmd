...
![analyse blockchain data](analyse-blockchain-data.jpeg)
...
用日常案例举例, 希望分析 `ETH` 某个地址交易信息, 首先需要通过三方 API 获取交易信息, 导入到 `DuckDB` 中, 然后通过 `SQL` 查询分析.


# 演示

这个过程在以往, 首先通过 `requests` 获取数据, 接着使用 `pandas` 做数据预览和数据清洗, 最后使用 `DuckDB` 进行分析.
在准备数据阶段有 3 个必要步骤
...
下面展示使用 `blockscout` API, 通过 `DuckDB UDF` 和 `DuckDB Macro` 简化这个过程.
...
通过 `blockscout` 提供的 API 获取 `ETH` 地址的交易信息, 代码如下:
...
注册 `DuckDB` 的自定义函数, 注意 page 和 offset 做了调整, 只获取 1 页面数据, 不做翻页演示.
```{python}
#| output: false
conn = duckdb.connect()
...
通过 `DuckDB` 的 `UDF` 和 `Macro` 可以简化数据处理的过程, 使得数据处理更加高效, 代码更加简洁.
实际使用过程, 推荐大家使用方案2, 严格字段约束, 避免数据处理过程中出现问题.

另外, 也可以通过 `CTE recursive` 实现翻页查询, 但是这个过程比较复杂, 代码量也比较大, 不推荐使用. 后续有时间会写篇文章分享.

# references
[Python Runction API - DuckDB](https://duckdb.org/docs/api/python/function)
[CREATE MACRO Statement - DuckDB](https://duckdb.org/docs/sql/statements/create_macro)
[Get Transactions By Address - Blockscout](https://docs.blockscout.com/for-users/api/rpc-endpoints/account#get-transactions-by-address)
